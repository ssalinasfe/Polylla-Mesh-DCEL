...
Others
    ...
    get_PointX(std::size_t i): return the i-th x coordinate of the triangulation
    get_PointY(std::size_t i): return the i-th y coordinate of the triangulation
...
struct vertex{
    ...
    std::size_t incident_halfedge; // halfedge incident to the vertex, vertex is the origin of the halfedge
};



struct halfEdge {
    std::size_t origin; //tail of edge
    std::size_t target; //head of edge
    std::size_t twin; //opposite halfedge
    std::size_t next; //next halfedge of the same face
    std::size_t prev; //previous halfedge of the same face
    std::size_t face = -1; //face index incident to the halfedge
    std::size_t is_border; //1 if the halfedge is on the boundary, 0 otherwise
};
...
public:
    ...
    std::size_t n_halfedges = 0; //number of halfedges
    std::size_t n_faces = 0; //number of faces
    std::size_t n_vertices = 0; //number of vertices
    ...
    std::vector<std::size_t> triangle_list; //list of edges that generate a unique triangles, 
...
private:
    ...
    std::vector<std::size_t> read_triangles_from_file(std::string name){
    ...
    void construct_interior_halfEdges(std::vector<std::size_t> faces, std::vector<int> neighs){
        for(std::size_t i = 0; i < n_faces; i++){
            halfEdge he0, he1, he2;
            std::size_t index_he0 = i*3+0;
            std::size_t index_he1 = i*3+1;
            std::size_t index_he2 = i*3+2;
            std::size_t v0 = faces.at(3*i+0);
            std::size_t v1 = faces.at(3*i+1);
            std::size_t v2 = faces.at(3*i+2);
            std::size_t n0 = neighs.at(3*i+0);
            std::size_t n1 = neighs.at(3*i+1);
            std::size_t n2 = neighs.at(3*i+2);
...
public:
    ...
    void print_pg(std::string file_name){
        ...
        for(std::size_t i = 0; i < n_vertices; i++){
            vertex v = Vertices.at(i);
            std::size_t incident = v.incident_halfedge;
            std::size_t curr = incident;
            std::size_t twin = HalfEdges.at(curr).twin;
            ...
            std::size_t nxt = CCW_edge_to_vertex(curr);
            while(nxt != curr){
    ...
    std::size_t face_index(std::size_t i){
    ...
    triangle incident_face(std::size_t e)
    {   
        triangle face;  
        std::size_t nxt = e;
        std::size_t init_vertex = origin(nxt);
    ...
    {
        std::size_t v0 = tr.at(0);
        std::size_t v1 = tr.at(1);
        std::size_t v2 = tr.at(2);
        double area = 0.0;
    ...
    std::size_t CCW_edge_to_vertex(std::size_t e)
    {
        std::size_t twn, nxt;
        if(is_border_face(e)){
    ...
    std::size_t CW_edge_to_vertex(std::size_t e)
    {
        std::size_t twn, prv, nxt;
    ...
    double get_PointX(std::size_t i){
        return Vertices.at(i).x;
    }

    double get_PointY(std::size_t i){
    ...
    std::size_t origin(std::size_t e)
    {
    ...
    std::size_t target(std::size_t e)
    {
    ...
    std::size_t edge_of_vertex(std::size_t v)
    {
    ...
    //         false otherwise
    bool is_interior_face(std::size_t e)
    {
    ...
    bool is_border_vertex(std::size_t v)
    {
